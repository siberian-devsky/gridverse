...
// DELETE /cells/delete/deletemany - delete many cells in a list
export const deleteManyCellsByName: RequestHandler = async (req, res) => {
    const cells: string[]  = req.body.data

    if ( !Array.isArray(cells) || !cells.every( c => typeof c === 'string') ) {
        res.status(400).json({
            status: 400,
            message: {
                m: "An array is required for the db",
                data: `we got ${cells} as ${typeof(cells)}`,
            },
        });
    ...
    try {
        const deleted = await prisma.basicCell.deleteMany({
            where: {
                name: { in: cells },
            },
        });
        
        console.log(`${deleted.count} records were deleted`);
        res.status(200).json({
            status: 200,
            message: `${deleted.count} records were deleted`,
            data: cells,
        });
    } catch (err) {
        if (err instanceof Prisma.PrismaClientKnownRequestError) {
